...
> механизмы, зачарование и зельеварение, полноценный ИИ и разнообразие мобов,
> и ещё ряд блоков/механик. Подробный список — в
> [`license/CHANGELOG_RU.md`](license/CHANGELOG_RU.md).
>
...
Цель проекта — быстрое и лёгкое ядро сервера с предсказуемым потреблением
ресурсов там, где официальный Java-сервер тратит время на старт JVM и много
памяти на heap. Это учебный/исследовательский проект, а не замена официальному
серверу.
...
Полный список — в [`license/CHANGELOG_RU.md`](license/CHANGELOG_RU.md).
...
| Употребление еды и эффекты от еды | ✅ |
...
> **Вообще не проверялось.** Сборка под Linux никогда не проверялась — возможно,
> она даже не собирается. Команды ниже — отправная точка, а не поддерживаемый путь.
...
## Игровые команды (скины и плащи)

Вводятся игроком в чате:

```
/skin <ник>               надеть скин лицензионного игрока
/skin reset               вернуть свой скин
/cape <id>                надеть плащ minecraft.net
/cape off                 снять плащ
/capes                    список доступных плащей
```

> **Скины и плащи работают только при `online-mode=false`.** При
> `online-mode=true` плащи намеренно отключены — выдача официальных плащей
> minecraft.net лицензионным аккаунтам нарушает Minecraft EULA, поэтому сервер
> их не выдаёт. `/skin` позволяет офлайн-игроку (без лицензии) надеть скин
> любого лицензионного ника; `/skin reset` возвращает свой. Если на тебе плащ,
> первый `/skin reset` только предупреждает — введи его ещё раз в течение 15
> секунд, чтобы сбросить и скин, и плащ.
...
- Доработка транспорта (лодки, вагонетки)
